...
`agents/mcp-servers/llmops-golden-paths-server/token_verifier.py` too — each MCP
server verifies its own tokens independently, on purpose, so one server's
bug/compromise can't forge trust for another.
...
separate "resource" audience to mint against.
"""
...
logger = logging.getLogger("mlops_golden_paths_server.token_verifier")
