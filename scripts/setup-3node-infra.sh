...
# (infra/argo-workflows/train-register-cluster-template.yaml) is now pinned
...
kubectl --context "$CTX" apply -f infra/k8s/ai-platform-zone/namespace.yaml
kubectl --context "$CTX" -n ai-platform-zone create configmap litellm-config \
  --from-file=config.yaml=infra/llm-gateways/litellm-config.yaml \
...
kubectl --context "$CTX" -n ai-platform-zone create secret generic litellm-secrets \
  ...
  --dry-run=client -o yaml | kubectl --context "$CTX" apply -f -
kubectl --context "$CTX" apply -f infra/k8s/ai-platform-zone/
...
kubectl --context "$CTX" apply -f infra/openchoreo/platform/clustercomponenttype-pinned-service.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/clusterrole-orchestration-api.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/networkpolicy-orchestration-api-workflows.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/resource-mlflow.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/resource-qdrant.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/resource-litellm.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/component-orchestration-api.yaml
kubectl --context "$CTX" apply -f infra/openchoreo/platform/workload-orchestration-api.yaml
for svc in observability-server llmops-golden-paths-server golden-path-guide-server mlops-golden-paths-server; do
  kubectl --context "$CTX" apply -f "infra/openchoreo/platform/component-${svc}.yaml"
  kubectl --context "$CTX" apply -f "infra/openchoreo/platform/workload-${svc}.yaml"
done
...
for svc in orchestration-api observability-server llmops-golden-paths-server golden-path-guide-server mlops-golden-paths-server; do
  kubectl --context "$CTX" apply -f "infra/openchoreo/platform/releasebinding-${svc}-development.yaml"
done
kubectl --context "$CTX" apply -f infra/argo-workflows/train-register-cluster-template.yaml
